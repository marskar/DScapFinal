...
The reason I broke the data down like this is to avoid reading too much of the data into memory at once.
The split method can work by number of characters, words, or line count.
 
```{r echo=TRUE, eval=TRUE, warning=FALSE, error=FALSE}

# Here I use a system command to split the data into 1MB chunks
system('split ./final/en_US/en_US.twitter.txt --bytes=1M -d --numeric-suffixes --suffix-length=3 --additional-suffix="twit.txt"')

system('split ./final/en_US/en_US.news.txt --bytes=1M -d --numeric-suffixes --suffix-length=3 --additional-suffix="news.txt"')

system('split ./final/en_US/en_US.blogs.txt --bytes=1M -d --numeric-suffixes --suffix-length=3 --additional-suffix="blog.txt"')
...
blog1 <- sample(blog, size = length(blog) / 1000, replace = FALSE)
news1 <- sample(news, size = length(news)/1000, replace = FALSE)
twit1 <- sample(twit, size = length(twit) / 1000, replace = FALSE)
...
dat2<-stripWhitespace(dat1)
?stripWhitespace
# Convert to ASCII and remove non-ASCII characters
dat3<-iconv(dat2, "UTF-8", "ASCII", sub="")
# make corpus
dat4 <- tolower(dat3)
...
## Remove profanity
datNoProfan <- removeWords(dat4, profan) 

## Find out the object size difference after removing profanity
object.size(dat4)
object.size(datNoProfan)
object.size(dat4)-object.size(datNoProfan)
```
...
uniList <- unigram_tokenizer(datNoProfan)
...
biListTest <- bigram_tokenizer(datNoProfan)
freqNamesTest <- as.vector(names(table(unlist(biListTest))))
freqCountTest <- as.numeric(table(unlist(biListTest)))
dfBiTest <- data.frame(Word = freqNamesTest,
                    Count = freqCountTest)
attach(dfBiTest)
dfBiSortTest<-dfBiTest[order(-Count),]
detach(dfBiTest)

trigram_tokenizer <- ngram_tokenizer(3)
triList <- trigram_tokenizer(datNoShort)
...
## Next steps
For my app, I am interested in providing functionality for hash tags from the twitter data. The idea is to predict what may follow a hash tag, just like other words. Hashtags by themselves are unigrams even if they represent multiple words (e.g. #HungryLikeAWolf), but they may be preceeded by other words. The predictive model would first try to predict by a quadgram, then a trigram, then a bigram and finally the word itself. 
